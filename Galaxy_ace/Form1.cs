...
namespace Galaxy_ace
{
    public partial class SpaceForm : Form
    {
        ...
        private void SpaceForm_Load(object sender, EventArgs e)
        {
            ...
            Explosion.settings.volume = 26;

            //Load all Images
            Image enemi1 = Image.FromFile("asserts\\h1.png");
            Image enemi2 = Image.FromFile("asserts\\h2.png");
            Image enemi3 = Image.FromFile("asserts\\h3.png");
            Image boss1 = Image.FromFile("asserts\\h4.png");
            Image boss2 = Image.FromFile("asserts\\h1.png");
            ...
            for (int i = 0; i < EnemisMunition.Length; i++)
            {
                ...
                EnemisMunition[i].Size = new Size(4, 15);
                EnemisMunition[i].Visible = false;
                EnemisMunition[i].BackColor = Color.OrangeRed;
                int x = rnd.Next(0, 10);
        ...
        private void ShowScore()
        {
            ...
            string constr = (@"Data Source=.\SQLEXPRESS;Initial Catalog=game_db;User Id=sa;Password=himel123$;Integrated Security=True");
            SqlConnection conn = new SqlConnection(constr);
            conn.Open();
            SqlCommand comm = new SqlCommand("select distinct Top 3 a.score from score a order by score DESC", conn);
            try
            {
                var TopScores = "Top Three Scores\n\n";

                SqlDataReader dreader = comm.ExecuteReader();
                while (dreader.Read())
                {
                    var score = dreader[0].ToString();
        ...
        {
            int scorevalue = Convert.ToInt32(scorelbl.Text);


            string constr = (@"Data Source=.\SQLEXPRESS;Initial Catalog=game_db;User Id=sa;Password=himel123$;Integrated Security=True");
            SqlConnection conn = new SqlConnection(constr);
            conn.Open();
            SqlCommand comm = new SqlCommand("insert into score values('" + scorevalue + "')", conn);
            try
            {
